...
var myDevice = "fortigate";



export default class Fortigate extends Component {
...
createServer1 = () => {
    const sshSrv = {msgId: 'term', ip: this.state.domaineName, username: "admin", password: "admin"};
  ...
  const { open, modale} = this.state;
    return (
      ...
      <Modal open={open} onAfterOpen={this.afterOpenModal}  onClose={() => this.onCloseModal()} blockScroll={!open} center="true">
        ...
        <div>
          <h1 className="MainPageTitle">Routeur Fortigate</h1>
        </div>
